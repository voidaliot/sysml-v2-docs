...
## Part 16 — Tool Rendering Notes

### Model as source of truth

Tools may be model-first (draw → model generates text), text-first (text → model generates diagram), or bi-directional. The spec requires semantic consistency across all representations. Text omits layout and internal IDs; diagrams omit syntactic detail. Both are views.

### PlantUML (OMG Pilot Implementation / Jupyter)

Used by the OMG Pilot Implementation. Partial conformance — layout is limited, Action Flow view is not fully implemented, port labels can be hard to read on complex interconnection views.

### SysON (Obeo / Eclipse Sirius Web)

The most conformant open-source graphical renderer as of 2025. Implements `gv`, `iv`, `av`, `stv`, and `bv`. Text editor integration via SysIDE. Available as a Docker image.

### PTC Modeler (10.x+)

Commercial. Full rendering support including bi-directional text/diagram round-tripping.

### Sensmetry Syside (VS Code extension)

Focused on textual LSP features as of April 2026. Diagram rendering planned for a future release.

### VS Code extension — rendering priority (Sprint 11)

1. General View `gv` — highest value; covers taxonomy, composition, requirements, allocations
2. State Transition View `stv` — maps directly to Mermaid `stateDiagram-v2`
3. Action Flow View `av` — maps to Mermaid `flowchart`; fork/join needs custom SVG
4. Sequence View `sv` — maps to Mermaid `sequenceDiagram`
5. Browser View `bv` — maps to a tree widget; straightforward
6. Grid View `grv` — maps to an HTML table; straightforward
7. Interconnection View `iv` — most complex; port stubs require custom geometry layout
8. Geometry View `gev` — deferred; requires a spatial layout engine
